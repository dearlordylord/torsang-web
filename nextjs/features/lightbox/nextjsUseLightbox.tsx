...
export default function useNextjsLightbox() {
  ...
  const getIndex = useCallback((slides: Slide[]) => {
  ...
  const render = React.useCallback(
    (props?: Omit<LightboxExternalProps, "open" | "close">) =>
      isOpen ? (
        <Lightbox open={isOpen} close={close} index={getIndex(props.slides)} render={{
          slide: LightboxNextJsImage,
        }} {...props} carousel={{
          ...(props.carousel ? props.carousel : {}),
          finite: typeof props.carousel?.finite === 'undefined' ? props.slides.length < 3 : props.carousel.finite,
        }} />
      ) : null,
